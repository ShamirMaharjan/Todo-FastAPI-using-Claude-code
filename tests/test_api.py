...
class TestHealthCheck:
    """Tests for the health-check and info endpoints."""
...
class TestTaskCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_task_with_all_fields(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_task_with_priority(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_task_default_priority(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_task_invalid_data(self, client: AsyncClient) -> None:
    ...
    async def test_get_task_by_id(self, client: AsyncClient) -> None:
        ...
        create_response = await client.post("/tasks/", json={"title": "Fix bug"})
    ...
    async def test_update_task(self, client: AsyncClient) -> None:
        ...
        create_response = await client.post("/tasks/", json={"title": "Old task"})
        task_id = create_response.json()["id"]
    ...
    @pytest.mark.asyncio
    async def test_update_task_priority(self, client: AsyncClient) -> None:
        """PATCH /tasks/{id} should update the priority field."""
        create_response = await client.post("/tasks/", json={"title": "Task"})
        task_id = create_response.json()["id"]
    ...
    @pytest.mark.asyncio
    async def test_update_task_not_found(self, client: AsyncClient) -> None:
        """PATCH /tasks/{id} with unknown id should return 404."""
        response = await client.patch("/tasks/9999", json={"title": "X"})
        assert response.status_code == 404
        assert response.json()["detail"] == "Task with id 9999 not found"
    ...
    @pytest.mark.asyncio
    async def test_delete_task_not_found(self, client: AsyncClient) -> None:
...
class TestTaskPagination:
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_pagination_limit(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_pagination_offset(self, client: AsyncClient) -> None:
        """GET /tasks/?offset=M should skip the first M tasks."""
        for i in range(5):
            await client.post("/tasks/", json={"title": f"Task {i}"})

        response = await client.get("/tasks/?offset=2")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 3
        titles = {item["title"] for item in data}
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_pagination_combined(self, client: AsyncClient) -> None:
        """GET /tasks/?limit=N&offset=M should return N tasks starting after M."""
        for i in range(5):
            await client.post("/tasks/", json={"title": f"Task {i}"})

        response = await client.get("/tasks/?limit=2&offset=1")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2
        titles = {item["title"] for item in data}
    ...
    @pytest.mark.asyncio
    async def test_list_tasks_pagination_default(self, client: AsyncClient) -> None:
    ...
    async def test_list_tasks_pagination_limit_exceeds_max(
        ...
        response = await client.get("/tasks/?limit=200")
        assert response.status_code == 422
...
class TestTaskFiltering:
    ...
    async def test_list_tasks_filter_completed_true(
        ...
        await client.post("/tasks/", json={"title": "Done task", "completed": True})
        await client.post("/tasks/", json={"title": "Pending task", "completed": False})
...
class TestTaskValidation:
    ...
    @pytest.mark.asyncio
    async def test_create_task_rejects_empty_title(self, client: AsyncClient) -> None:
    ...
    async def test_create_task_rejects_title_too_long(
        ...
        response = await client.post("/tasks/", json={"title": "x" * 256})
        assert response.status_code == 422
